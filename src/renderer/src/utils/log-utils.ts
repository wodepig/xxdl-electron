...
export class LogFileWatcher extends EventEmitter {
  ...
  async initWatch(): Promise<Array<string>> {
    try {
      // 1. 先读取前50条日志并返回
      const first50Logs = await this.readFirstNLogs(50);
   ...
   */
  private async readFirstNLogs(n: number): Promise<Array<string>> {
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.stat(this.filePath, (err, stats) => {
        ...
        if (newSize < this.fileSize) {
          this.fileSize = 0;
          this.readFirstNLogs(50).then((logs) => {
